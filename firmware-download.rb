...
def main()
	...
	# This is only required if changing fw_customer_id ( i.e. MS to MT ) , otherwise set to nil
	fw_customer_id = nil
	...
	if method == 'angel'
		...
		$ANGEL.firmware_download( firmware: firmware , fw_customer_id: fw_customer_id , firmware_slot: nil , power_cycle: power_cycle , commit_action: commit_action )
	else
		$ANGEL.firmware_download_nvme_cli( firmware: firmware , fw_customer_id: fw_customer_id , firmware_slot: nil , power_cycle: power_cycle , commit_action: commit_action )
	end
